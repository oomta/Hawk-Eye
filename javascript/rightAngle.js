var rightFrame = function (idx) {
    ...
    box.on('dragend', function () {
        ...
        var error = Math.abs(curX < x1 ? 90 + ((curY <= y1 ? -1 : 1) * curAngle) + angle : 90 + ((curY <= y1 ? -1 : 1) * curAngle) - angle) * 2.0;
        globalError = parseFloat(globalError) + parseFloat(error);
        error = (error + 0.0005).toFixed(3);
        showError(error);
        setScore(idx, error);
