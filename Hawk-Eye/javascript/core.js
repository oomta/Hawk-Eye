...
function showError(error) {
    var intPart = Math.floor(error);
    var floatPart = "" + Math.floor((error - intPart) * 1000.0 + 0.5);
...
function setScore(idx, error) {
    jQuery('#score-board li:eq(' + idx + ')').html(error);
    count += 1;
    jQuery('#avg-error').html((globalError / count + 0.00005).toFixed(3));
}
